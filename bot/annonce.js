/* Publie une annonce au nom du bot dans un salon Discord.
   ...
   accord sur le texte exact. Jamais de publication de test.
     ...
     node bot/annonce.js --message chemin/vers/message.txt [--image chemin.png] [--salon <id>] [--a-blanc]
...
if (!TOKEN || !SALON || !FICHIER) {
    console.error('Usage : node bot/annonce.js --message fichier.txt [--image image.png] [--salon id] [--a-blanc]');
    process.exit(1);
}
...
console.log('Salon : ' + SALON + (IMAGE ? ' | image : ' + path.basename(IMAGE) + ' (' + Math.round(fs.statSync(IMAGE).size / 1024) + ' Ko)' : ' | sans image'));
console.log('----- message (' + texte.length + ' caracteres) -----\n' + texte + '\n----- fin -----');
if (A_BLANC) { console.log('\nMode a blanc : rien n a ete envoye.'); process.exit(0); }

(async () => {
    const payload = { content: texte, allowed_mentions: { parse: ['everyone'] } };
